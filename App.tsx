...
import { VoiceGestureControl } from './components/VoiceGestureControl';
...
export function App() {
  ...
  const [simInstance, setSimInstance] = useState<MujocoSim | null>(null);
  ...
  useEffect(() => {
      ...
      try {
          const sim = new MujocoSim(containerRef.current, mujocoModuleRef.current);
          simRef.current = sim;
          sim.renderSys.setDarkMode(isDarkMode);
          
          sim.init("franka_panda_stack", "scene.xml", (msg) => {
